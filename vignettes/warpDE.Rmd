...
This vignette describes the workflow of the R package `warpDE`, showing three different methods to infer differentially expressed (DE) genes from reconstructed cell lineages. After a first step of lineage inference and cell pseudotime ordering performed with `Slingshot` (see [@Street2017]) @Street2017, we try to find genes with significantly different behaviors between two cell lineages.

This vignette uses the data from [@Fletcher2017], which examines the differentiation of stem cells from the mouse olfactory epithelium. For the sake of simplicity, we restricted the data to only two lineages: the lineages corresponding to the differentiation of horizontal basel cells (HBC) into neuronal and sustentacular cells, respectively.
